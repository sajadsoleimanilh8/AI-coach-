...
| "Simulation uses reinforcement learning" | No. `ai/simulation_ai/what_if_analysis/engine.py` is a **deterministic recompute engine** — it reruns the same scoring functions with one changed parameter. `train_simulation.py` is a literal one-line TODO. |
