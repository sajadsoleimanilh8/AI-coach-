...
class Video(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    ...
    match_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("matches.match_id"), nullable=True, index=True)
...
class AnalysisResult(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    job_id: Mapped[str] = mapped_column(String(36), ForeignKey("processing_jobs.id"), nullable=False, unique=True, index=True)
...
class Match(Base):
    ...
    match_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
...
class Frame(Base):
    ...
    frame_id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True, nullable=False)
    match_id: Mapped[str] = mapped_column(String(36), ForeignKey("matches.match_id"), nullable=False, index=True)
...
class PlayerDetection(Base):
    ...
    detection_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    frame_id: Mapped[int] = mapped_column(Integer, ForeignKey("frames.frame_id"), nullable=False, index=True)
    player_id: Mapped[int] = mapped_column(Integer, nullable=False)
    team_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    team_assignment_confidence: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class BallDetection(Base):
    ...
    detection_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    frame_id: Mapped[int] = mapped_column(Integer, ForeignKey("frames.frame_id"), nullable=False, index=True)
    ball_x: Mapped[float] = mapped_column(Float, nullable=False)
    ball_y: Mapped[float] = mapped_column(Float, nullable=False)
    confidence: Mapped[float] = mapped_column(Float, nullable=False)

class PlayerTracking(Base):
    __tablename__ = "player_tracking"
    tracking_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    match_id: Mapped[str] = mapped_column(String(36), ForeignKey("matches.match_id"), nullable=False, index=True)
    player_id: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    frame_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)  # actual video frame number, NOT a frames.frame_id FK
    team_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    pixel_x: Mapped[float] = mapped_column(Float, nullable=False)
    pixel_y: Mapped[float] = mapped_column(Float, nullable=False)
    pitch_x_m: Mapped[float | None] = mapped_column(Float, nullable=True)
    pitch_y_m: Mapped[float | None] = mapped_column(Float, nullable=True)
    ...
    body_orientation_deg: Mapped[float | None] = mapped_column(Float, nullable=True)         # shoulder-line angle, 0-360
    body_orientation_confidence: Mapped[float | None] = mapped_column(Float, nullable=True)  # min visibility of the two shoulder landmarks, 0-1

    match = relationship("Match", back_populates="player_trackings")

class Event(Base):
    __tablename__ = "events"
    event_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    match_id: Mapped[str] = mapped_column(String(36), ForeignKey("matches.match_id"), nullable=False, index=True)
    frame_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("frames.frame_id"), nullable=True, index=True)
...
class PlayerMetric(Base):
    ...
    metric_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    match_id: Mapped[str] = mapped_column(String(36), ForeignKey("matches.match_id"), nullable=False, index=True)
    player_id: Mapped[int] = mapped_column(Integer, nullable=False)
    metric_name: Mapped[str] = mapped_column(String(120), nullable=False)
    value: Mapped[float | None] = mapped_column(Float, nullable=True)
    method: Mapped[MetricMethod] = mapped_column(Enum(MetricMethod), nullable=False)
    confidence: Mapped[MetricConfidence] = mapped_column(Enum(MetricConfidence), nullable=False)
    sample_size: Mapped[int] = mapped_column(Integer, nullable=False)
    sub_scores: Mapped[Any] = mapped_column(JSON, nullable=False)
...
class CalibrationStatus(Base):
    ...
    calibration_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    match_id: Mapped[str] = mapped_column(String(36), ForeignKey("matches.match_id"), nullable=False, index=True)
    ...
    frame_start: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    ...
    confidence: Mapped[float] = mapped_column(Float, nullable=False)
    reprojection_error_m: Mapped[float | None] = mapped_column(Float, nullable=True)
    n_points: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    source: Mapped[CalibrationSourceKind] = mapped_column(Enum(CalibrationSourceKind), nullable=False)

    # The frame the homography was actually solved on -- equal to
    # frame_start for a fresh solve, EARLIER than it for a carried one.
    solved_on_frame: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    camera_motion: Mapped[str | None] = mapped_column(String(16), nullable=True)
    camera_shift_px: Mapped[float | None] = mapped_column(Float, nullable=True)
    ...
    homography_matrix: Mapped[Any] = mapped_column(JSON, nullable=True)
...
class PreMatchQuestionnaire(Base):
    ...
    # out-of-range answer.
    questionnaire_json: Mapped[Any] = mapped_column(JSON, nullable=False)
...
class PreMatchHealthAssessment(Base):
    ...
    submission_index: Mapped[int] = mapped_column(Integer, nullable=False, default=1, index=True)
    ...
    physical_readiness: Mapped[float] = mapped_column(Float, nullable=False)
    ...
    performance_risk: Mapped[RiskLevel] = mapped_column(Enum(RiskLevel), nullable=False)
    workload_risk: Mapped[RiskLevel] = mapped_column(Enum(RiskLevel), nullable=False)
...
class PsychologyAssessment(Base):
    ...
    assessment_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id, nullable=False)
    ...
    cv_player_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    responses: Mapped[Any] = mapped_column(JSON, nullable=False)
    ...
    mental_readiness: Mapped[float] = mapped_column(Float, nullable=False)
    ...
    pressure_risk: Mapped[str] = mapped_column(String(16), nullable=False)
    mental_performance_risk: Mapped[str] = mapped_column(String(16), nullable=False)
    ...
    confidence_level: Mapped[MetricConfidence] = mapped_column(Enum(MetricConfidence), nullable=False)
